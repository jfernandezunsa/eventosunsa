...
import { Menu, SquareMenu, SquareX, X } from 'lucide-react';
...
const MenuMain = () => {
  ...
  let [open, setOpen]=useSate(false);
  ...
  return (
    <div className="sm:flex place-content-between items-center w-full lg:max-w-[1200px] m-auto bg-white p-6 border-b-2 border-[#051F43] shadow-lg">
          ...
          <div onClick={()=>setOpen(!open)} className='absolute right-8 top-8 cursor-pointer md:hidden'>
          {open ? (<Menu size={36} color="#051F43" />) : (<X size={36} color="#051F43"/>)}
          {/* <Menu size={36} color="#051F43" /><X size={36} color="#051F43"/> */}
          </div>
          <ul className="md:flex gap-5 items-center mt-10 sm:mt-0 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto pb-6 md:pb-0 pl-6 md:pl-0 transition-all duration-500 ease-in">
          {
            Links.map((Link)=>(
